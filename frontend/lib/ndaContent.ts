...
/**
 * Content model for the Common Paper Mutual NDA (Standard Terms v1.0), adapted
 * from templates/Mutual-NDA.md and templates/Mutual-NDA-coverpage.md at the
 * repo root. The Standard Terms are invariant boilerplate; `{{ref}}` tokens
 * mark the points where the original template cross-references a Cover Page
 * value (via a `coverpage_link` span) rather than substitute text inline.
 */
...
export type ContentPart =
  ...
  | { kind: "ref"; key: RefKey; label: string };

const TOKEN_RE = /\*\*(.+?)\*\*|\{\{(\w+)\}\}/g;

export function tokenize(template: string): ContentPart[] {
  ...
  while ((match = TOKEN_RE.exec(template))) {
    ...
    if (match[1] !== undefined) {
      ...
      const key = match[2] as RefKey;
      parts.push({ kind: "ref", key, label: REF_LABELS[key] });
    }
...
export function formatDisplayDate(isoDate: string): string {
  if (!isoDate) return "[Effective Date not yet provided]";
  const date = new Date(`${isoDate}T00:00:00`);
  if (Number.isNaN(date.getTime())) return "[Effective Date not yet provided]";
...
export function resolveCoverPageValues(
  data: NdaFormData,
): Record<RefKey, string> {
  return {
    purpose: data.purpose.trim() || "[Purpose not yet provided]",
    effectiveDate: formatDisplayDate(data.effectiveDate),
    mndaTerm: formatMndaTerm(data),
    confidentialityTerm: formatConfidentialityTerm(data),
    governingLaw: data.governingLaw.trim() || "[Governing Law not yet provided]",
    jurisdiction: data.jurisdiction.trim() || "[Jurisdiction not yet provided]",
  };
}

export const STANDARD_TERMS_SECTIONS: string[] = [
