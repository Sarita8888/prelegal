...
- **Deployment note**: in addition to the documented single-container Docker deployment, there are also two separate Vercel projects (`frontend`, `backend`) connected to this repo, each auto-deploying preview URLs per branch/PR plus a production deployment from `main`. These are NOT same-origin like the Docker setup, so the frontend project needs `NEXT_PUBLIC_API_BASE_URL` set (in Vercel project → Settings → Environment Variables) to the backend project's URL for chat/catalog calls to work at all — as of PA-6 this was still unset for Preview, so Vercel previews of the frontend alone won't reach a backend until it's configured. Backend CORS was widened (see PA-6 above) to permit these Vercel origins once that's set.